...
void MainWindow::findUser(const QString &name)
{
    ...
    requestId[GetUser] = flickr->get(method,request);
}
...
void MainWindow::requestFinished ( int reqId, FlickrResponse data, FlickrError err, void* userData )
{
    if ( !err.code )
    {
        if(requestId[GetUser] == reqId){
            ...
            FlickrMethod method("flickr.people.getPublicPhotos");
            method.args.insert("user_id",userid);
            method.args.insert("extras","url_s");
            method.args.insert("per_page","6");
            FlickrRequest request("photo","url_s");
            requestId[GetPhotos] = flickr->get(method,request);
        }else if(requestId[GetPhotos] == reqId){
            QList<FlickrTag> photos = data.tags.values("photo");
...
QString MainWindow::constructHtml(const QList<FlickrTag> &photos)
{
